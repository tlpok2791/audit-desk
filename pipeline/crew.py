...
def build_crew(s: Settings, row: dict[str, str], doc_text: str,
    ...
    t3 = Task(
        description=(
            ...
            "  강조 → 하지 않는다. 중요한 것은 문장 순서와 위치로 드러낸다.\n"
            "  나열 → 기호 없이 줄바꿈, 또는 '첫째, 둘째' / '①, ②' 같은 평문 기호.\n"
            "  링크 → 주소를 평문 그대로.\n"
            "아래 === 구분자 === 는 도구가 읽는 표식이라 예외다.\n\n"

            "■ 제목 후보 3개\n"
            "28자 내외. 핵심 키워드를 앞쪽에 둔다(검색 노출에서 앞 단어 가중치가 크다).\n"
            "질문형 / 정보형 / 대상 특정형처럼 서로 다른 각도로 뽑는다. "
            "같은 말을 바꿔 쓴 3개는 쓸모없다.\n\n"

            "■ 본문\n"
            "소제목 단위로 나누고 한 문단은 3~4줄 이내로 짧게 끊는다(모바일에서 벽처럼 보이면 안 된다).\n"
            "문단 사이에 빈 줄을 둔다.\n"
            "이미지가 들어가면 좋을 자리에 [이미지: 무엇을 담을지] 를 별도 줄로 넣는다. "
            "도입부 1개, 본문 중간 1~2개.\n\n"
            ...
            "필요합니다'를 덧붙인다.\n"
            "- 절세 결과를 보장하는 표현('무조건 절세', '100% 인정')은 쓰지 않는다.\n"
            "- 특정 병의원 홍보, 치료 효과 단정, 환자 후기 인용은 쓰지 않는다 "
            "(의료광고 심의 대상이 될 수 있다).\n"
            "- 근거 조문을 확실히 모르면 아예 쓰지 마라. 지어내지 마라.\n"
            "- 본문 맨 끝에 넣는다: '이 글은 일반적인 정보 제공을 목적으로 작성되었으며, "
            "특정 사안에 대한 세무 자문이 아닙니다. 실제 적용은 개별 사실관계와 신고 시점의 "
            "세법에 따라 달라질 수 있으므로, 구체적인 사안은 별도 검토가 필요합니다.'\n\n"

            "■ 태그 10개 — 핵심 키워드 + 연관 검색어 + 대상 독자(개원의, 병원장)를 섞어 쉼표로.\n\n"

            "■ 리포트 — 본문 총 글자수(공백 포함)와, 제목에 쓴 핵심 키워드가 본문에 "
            "몇 번 나오는지.\n\n"

            "■ 검토 필요 — 사람이 확인해야 할 것을 남긴다. 없으면 그 구획을 비운다.\n"
            "  [근거없음] 조문 확인이 필요한 대목\n"
            "  [수치] 판독불가였거나 확신이 없는 숫자\n"
            "  [의료광고] 심의 대상일 수 있는 문구\n\n"

            "출력 형식 — 아래 여섯 구분자를 그대로 지켜라. 순서도 바꾸지 마라.\n"
...
SECTIONS = ["진단리포트", "제목후보", "본문", "태그", "리포트", "검토필요"]

# 예전 구획명으로 나오는 경우를 흡수한다
...
def check_naver_ready(body: str) -> list[str]:
    ...
    """
    problems = []
    text = re.sub(r"^\[이미지:.*?\]\s*$", "", body or "", flags=re.M)
