...
고객 사례를 쓸 때는 거래처가 특정되지 않게 한다. 상호·업종·지역·금액이 겹치면
한 곳으로 좁혀진다. 유형으로만 쓰고 금액은 둥근 가상의 숫자로 바꾼다.
...
### 사다리 — 코드를 쓰기 전에 (ponytail)

첫 칸에서 걸리면 거기서 멈춘다.

1. **이게 있어야 하긴 한가?** 추측성 필요면 안 만들고, 안 만들었다고 한 줄 적는다
2. **이미 이 저장소에 있나?** 몇 파일 건너에 있는 걸 다시 짜는 게 제일 흔한 낭비다
3. **표준 라이브러리에 있나?**
4. **플랫폼 기본 기능으로 되나?** CSS 로 될 걸 JS 로 짜지 않는다
5. **이미 깐 의존성으로 되나?** 몇 줄로 될 일에 새 패키지를 넣지 않는다
6. **한 줄로 되나?**
7. 그제서야 최소한의 코드

**게으름은 효율이지 부주의가 아니다.** 아래는 절대 줄이지 않는다.

- 신뢰 경계의 입력 검증 — `/from-docs` 의 고객정보 유출 점검이 그 자리다
- 데이터를 잃게 하는 오류 처리
- 보안 조치
- 사용자가 명시적으로 요청한 것

**문제를 이해하는 데는 게으르지 않는다.** 사다리는 답을 짧게 하지 읽기를 짧게 하지
않는다. 이해를 건너뛴 작은 diff 는 효율처럼 보이는 자신만만한 오답이다.

일부러 자른 모서리에는 `ponytail:` 주석으로 한계와 올릴 조건을 적어 둔다.

> 2026-09, 이 저장소에서 14,677줄을 지웠다. 1번 칸을 매번 밟았으면
> 대부분 태어나지 않았을 줄이다.
