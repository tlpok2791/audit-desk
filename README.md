...
| IT-100 | 종합소득세 계산 | 수입금액(엑셀/직접입력) + 필요경비(영수증 PDF/장부/경비율) + 공제 항목 | 사업소득금액~차감납부세액 계산 결과 엑셀 (참고용 추정치) |
...
core/theme.py       색 · 서체
core/income_tax.py  IT-100 종합소득세 계산 엔진 (순수 함수) · 세율은 config/income-tax-2025.json
core/receipt.py     영수증·원천징수영수증 PDF에서 금액 후보를 뽑는 최선추정 파서 (PC 설치 전용)
modules/edit.py     JL-100 분개장·계정별원장 편집 (화면 하나, 자료 종류만 선택)
modules/            기능별 화면
build_web.py        tool.html 생성기
tax.html            종합소득세 계산 — 사장님이 직접 쓰는 고객용 화면 (아래 '고객용 화면' 참고)
src/tax/calc.js     세액 계산 엔진 (브라우저용) · core/income_tax.py 와 같은 계산
src/tax/wizard.js   단계별 화면 흐름 · 입력 수집
src/data/tax-rates.js  세율 상수 (build_tax.py 결과물 · 직접 고치지 말 것)
build_tax.py        config/income-tax-2025.json → src/data/tax-rates.js 생성기
...
같은 계산기를 두 벌 둔다. **쓰는 사람이 다르기 때문이다.**

| | `tax.html` (고객용) | Streamlit 모듈 (내부용) |
|---|---|---|
| 쓰는 사람 | 자영업자 사장님 본인 | 나 (회계사) |
| 여는 법 | 링크 하나 · 모바일 | `streamlit run app.py` |
| 말투 | "작년에 얼마나 버셨나요?" | 수입금액 · 필요경비 · 기납부세액 |
| 흐름 | 질문 하나씩 4단계 | 한 화면에 전부 |
| PDF 영수증 | 안 됨 (직접입력·경비율로 대체) | 됨 (`pdfplumber`) |

계산 로직은 `src/tax/calc.js`(브라우저)와 `core/income_tax.py`(파이썬) 두 벌이지만,
**세율·공제 상수는 `config/income-tax-2025.json` 하나뿐이다.** 파이썬은 JSON을 직접 읽고,
브라우저용 `src/data/tax-rates.js`는 `build_tax.py`가 같은 JSON에서 굽는다. 세법이 바뀌면
JSON만 고치고 `python3 build_tax.py`를 돌리면 양쪽에 함께 반영된다.

### 고객용 화면 (`tax.html`)

사장님은 `streamlit run` 을 하지 않는다. 링크를 열 뿐이고, 대개 밤 11시에 폰으로 연다.
그래서 내부 조서 도구와 다른 원칙으로 만들었다.

- **질문 하나씩.** 폼 네 덩어리를 한 번에 쏟지 않고 매출 → 경비 → 가족 → 기납부 순으로 묻는다
- **세무 용어를 쓰지 않는다.** "기납부세액" 대신 "이미 뗀 세금이 있으신가요?"
- **입력하는 동안 하단에 예상 세금이 계속 보인다.** 지금 뭘 하는 중인지 알 수 있게
- **금액은 치는 즉시 콤마와 "8,000만원" 읽기가 붙는다.** 0을 하나 더 친 걸 바로 알아채도록
- **결과는 큰 숫자 하나.** 계산 명세는 "왜 이 금액인가요?"를 눌러야 펼쳐진다
- 경비는 총액·항목별·업종 경비율 셋 중에 고른다

`vendor/xlsx.full.min.js`로 홈택스 엑셀을 브라우저에서 바로 읽고, 결과도 같은 라이브러리로
내려받는다. 서버가 없으므로 매출·경비가 밖으로 나가지 않는다.

```bash
node scripts/tax.test.js     # 세율구간 경계값 · 공제 중복배제 · 표시 유틸 검증
python3 build_tax.py         # 세법 개정 후 세율 상수 다시 굽기
```

### 내부용 Streamlit 모듈

사업소득자(개인)의 종합소득세를 **참고용으로 추정**한다. 실제 신고서를 대신하지
않으며, 가산세와 보험료·의료비·교육비 같은 특별세액공제는 범위 밖이다. 실제 신고 전
세무사 확인이 필요하다는 경고를 화면에 계속 띄운다.

**입력**
- 수입금액 — 홈택스에서 내려받은 매출자료 엑셀을 올려 열을 골라 합산하거나 직접 입력
- 필요경비 — 셋 중 하나: 영수증·세금계산서 PDF 업로드(합계금액 후보 추출) · 직접 입력
  장부(행 추가/편집) · 경비율(%) 직접 입력으로 추정
- 인적공제 — 기본공제 인원, 경로우대·장애인 인원, 부녀자/한부모(중복 배제), 자녀세액공제
  인원, 국민연금 등 공적연금 본인부담분
- 기납부세액 — 원천징수영수증 PDF 업로드(소득세 후보 추출) 또는 직접 입력

**PDF 업로드는 PC 설치 실행 전용이다.** `core/receipt.py`가 `pdfplumber`로 텍스트를 뽑아
"합계금액"·"원천징수세액" 류 키워드 주변 숫자를 후보로 보여줄 뿐 자동 확정하지 않는다 —
`config/standard-coa.json` 매핑과 같은 원칙. 브라우저(`tool.html`)에서는 PDF 업로드 UI가
자동으로 숨겨지고 직접입력만 쓸 수 있다.

세율·공제 상수는 `core/income_tax.py`가 아니라 `config/income-tax-2025.json`에 있다.
세법이 바뀌면 이 파일만 고치면 된다.

```bash
pytest tests/test_income_tax.py     # 세율구간 경계값 · 공제 중복배제 · 계산 흐름 회귀 테스트
```
